...
describe('Pathfinding Algorithms', function(){
  context('Helper functions', function(){
    it('getNodeWithLeastTraversalCost returns the node with the least tentative cost from a collection of nodes', function(){
      var node1 = new Node('1');
      node1.setTraversalCost(100);
      var node2 = new Node('2');
      node2.setTraversalCost(1);
      var node3 = new Node('3');
      node3.setTraversalCost(10);
      expect(getNodeWithLeastTraversalCost([node1, node2, node3])).to.deep.equal(node2)
    });
    it('findShortestPath returns an array of contiguous nodes sorted from startNode to endNode', function(){
      ...
      graph.start.setTraversalCost(0)
      graph.node2.setTraversalCost(1);
      graph.node3.setTraversalCost(3);
      graph.node4.setTraversalCost(2);
      graph.node5.setTraversalCost(3);
      graph.end.setTraversalCost(3);
    ...
    it('findShortestPath doesn\'t get stuck in an infinite loop with same-cost nodes', function(){
      ...
      var map = createMap(n);
      (function setNeighborsTraversalCost(node){
        if(typeof node.traversalCost === 'undefined'){
          node.setTraversalCost(1);
          node.neighbors.forEach(setNeighborsTraversalCost);
        }
      })(map.start);
  ...
  context('Dijkstra\'s Algorithm between two points',function(){
    ...
    it('assigns traversal costs to all nodes on the path', function(){
      var graph = createSimpleGraph();
      var path = dijkstra(graph.start, graph.end);
      expect(
        path.reduce(function(wereAllVisited, node){
          return wereAllVisited && node.traversalCost !== Infinity;
        }, true)
      ).to.be.true;
    })
...
describe('Node', function(){
  it('creates a new node', function(){
    ...
    expect(node.traversalCost).to.equal(Infinity);
  });
  it('can set traversalCost', function(){
    var node = new Node();
    expect(node.traversalCost).to.equal(Infinity);
    node.setTraversalCost(4);
    expect(node.traversalCost).to.equal(4);
  });
